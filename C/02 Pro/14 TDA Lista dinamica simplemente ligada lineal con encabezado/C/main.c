...
int main()
{
    ...
    imprimeLista(lis);//la lista ya es un apuntador
    pausar();
    insertable.clave=26;
    ...
    p=localiza(&buscable,lis);//la lista ya es un apuntador
    recuperado=*recupera(p);//ya no necesita recibir la lista
    printf("\nEliminando elemento en posicion p=%ld...\n",(long)p);
    suprime(p);
    printf("Insertando al principio de la lista...\n");
    inserta(&insertable,primero(lis));
    pausar();
    printf("Imprimiendo lista:\n");
    imprimeLista(lis);
