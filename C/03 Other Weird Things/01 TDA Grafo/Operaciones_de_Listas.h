...
TipoElemento* recupera(posicion p){
    ...
    if (nodo!=NULL){
        ...
        error("Solicita recuperar de un nodo que no existe");
        return NULL;
    }
}
...
void suprime(posicion p){
    ...
    if (nodo!=NULL){
        ...
        error("Solicita suprimir un nodo que no existe");
    }
}
...
posicion siguiente(posicion p){
    posicion nodo=p->sig;
    if (nodo==NULL)
        error("Posicion siguiente no existe");
    return nodo;
}
...
posicion anterior(posicion p,Lista l){
    ...
    if (ant==NULL)
        error("Posicion anterior no existe");
    return ant;
}
